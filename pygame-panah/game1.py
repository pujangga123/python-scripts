...
## CUSTOM
default_enemy_interval = 200
enemy_inteval = default_enemy_interval
...
## CUSTOM SOUND
killing_spree_sound = pygame.mixer.Sound("resources/audio/killing-spree.mp3")
dominating_sound = pygame.mixer.Sound("resources/audio/killing-spree.mp3")
megakill_sound = pygame.mixer.Sound("resources/audio/mega-kill.mp3")
unstoppable_sound = pygame.mixer.Sound("resources/audio/unstoppable.mp3")
...
while(running):
    ...
    screen.blit(score_text, (300, 10))  # Posisi skor di kiri atas
    ...
    if enemy_timer == 0:
        ...
        enemy_timer = randint(1, enemy_inteval)
    ...
    for enemy in enemies:
        ...
        for bullet in arrows:
            ...
            if enemy_rect.colliderect(bullet_rect):
                ...
                kill_count += 1
                if kill_count == 10:
                    killing_spree_sound.play()
                if kill_count == 15:
                    dominating_sound.play()
                if kill_count == 20:
                    megakill_sound.play()
                if kill_count == 25:
    ...
    # 9. Move the player ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if keys["top"]:
        playerpos[1] -= 2  # kurangi nilai y
    elif keys["bottom"]:
        playerpos[1] += 2  # tambah nilai y 

    if keys["left"]:
        playerpos[0] -= 2  # kurangi nilai x
    elif keys["right"]:
        playerpos[0] += 2  # tambah nilai x

    # Cek agar pemain tidak keluar dari layar
    playerpos[0] = max(0, min(playerpos[0], width - player.get_width()))  # Batas kiri dan kanan
    playerpos[1] = max(0, min(playerpos[1], height - player.get_height()))  # Batas atas dan bawah
